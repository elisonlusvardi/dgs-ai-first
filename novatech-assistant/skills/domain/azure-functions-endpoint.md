...
const RequestSchema = z.object({ query: z.string().min(1) });
const ResponseSchema = z.object({ answer: z.string(), sourceDocument: z.string() });

export async function queryHandler(requestBody: unknown, executeQuery: (query: string) => Promise<unknown>) {
	const input = RequestSchema.parse(requestBody);
	const serviceOutput = await executeQuery(input.query);
	return ResponseSchema.parse(serviceOutput);
}
```
...
Exemplo DON'T (TypeScript):
```typescript
export async function queryHandler(requestBody: any) {
	console.log("request", requestBody);
	// Anti-padrão: handler mistura validação fraca, regra de negócio e integração externa.
	if (!requestBody || !requestBody.query) {
		throw new Error("invalid request");
	}

	const raw = await fetch("https://example.external/endpoint", {
		method: "POST",
		body: JSON.stringify({ q: requestBody.query }),
	});
